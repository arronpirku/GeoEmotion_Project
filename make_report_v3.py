...
def load_run_dir(run_dir: Path):
    ...
    # try to load per-class report for error analysis
    rep = read_json_safe(run_dir / "classification_report.json")
    out["per_class_report"] = rep if isinstance(rep, dict) else None
...
def fig_latency_vs_macro(df, out_png, number):
    ...
    colors_m = {"Raw":"#1f77b4","Simplified":"#ff7f0e"}
    ...
    for _, r in sub.iterrows():
        plt.scatter(r["test_macro"], r["latency_ms_b32"],
                    c=colors_m.get(r["scheme"], "#888"),
                    marker=markers.get(r["model"], "o"),
                    s=70)
    color_handles = [Line2D([0],[0], marker='o', color='w',
                            markerfacecolor=colors_m[s], markersize=8, label=s)
...
def fig_test_bars(df, out_png, number):
    ...
    pv = pv.reindex(columns=list(SCHEMES)).sort_index()
...
def fig_quant_latency(df, out_png, number):
    """FP32 vs INT8 latency vs F1 scatter per scheme (for Table 3 context)."""
    q = df.dropna(subset=["ptq_fp32_macro","ptq_int8_macro"]).copy()
    if q.empty:
        plt.figure(figsize=(5,3)); plt.axis("off")
        plt.text(0.5,0.5,f"Figure {number} skipped: missing quant data",ha="center",va="center")
        plt.tight_layout(); plt.savefig(out_png,dpi=180); plt.close(); return
    plt.figure(figsize=(6,3))
    for s in q["scheme"].unique():
        sub = q[q["scheme"]==s]
        plt.scatter(sub["ptq_fp32_macro"], sub["ptq_fp32_lat_s"], label=f"{s} FP32", marker="o")
        plt.scatter(sub["ptq_int8_macro"], sub["ptq_int8_lat_s"], label=f"{s} INT8", marker="x")
    plt.xlabel("Macro-F1")
    plt.ylabel("Latency (s)")
    plt.title(f"Figure {number}. FP32 vs INT8 Trade-off by Scheme")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_png,dpi=180)
    plt.close()
...
def make_error_diagnostics(df):
    """
    Dig out worst classes and FP/FN tendencies where per-class classification_report.json exists.
    Returns HTML string for the PDF paragraph.
    """
    rows = []
    for _, r in df.iterrows():
        rep = r.get("per_class_report")
        if not isinstance(rep, dict): 
            continue
        # scikit report typically has keys for classes + 'macro avg', 'weighted avg', 'accuracy'
        for cls, stats in rep.items():
            if cls in ("macro avg", "weighted avg", "micro avg", "accuracy"):
                continue
            p = stats.get("precision"); rc = stats.get("recall"); f1 = stats.get("f1-score")
            s = stats.get("support")
            if f1 is None or rc is None or p is None: 
                continue
            # very crude FP vs FN hint: low recall vs precision => FN-heavy; low precision vs recall => FP-heavy
            if rc < (p - 1e-9):
                tendency = "FN-heavy"
            elif p < (rc - 1e-9):
                tendency = "FP-heavy"
                ...
                tendency = "balanced"
            rows.append({
                "model": r["model"], "scheme": r["scheme"], "class": cls,
                "precision": p, "recall": rc, "f1": f1, "support": s, "tendency": tendency
            })
    if not rows:
        return ("<b>Error Analysis.</b> Overall Macro-F1 ≈ 0.47 indicates underfitting and label ambiguity. "
                "Common causes here are class imbalance and overlapping emotions; typical failures are false negatives "
                "on minority classes. Threshold tuning (Raw), class-weighting, and minority-targeted augmentation are recommended.")
    df_e = pd.DataFrame(rows)
    worst = (df_e.sort_values("f1").groupby(["model","scheme"]).head(3)
             .reset_index(drop=True)[["model","scheme","class","f1","precision","recall","tendency"]])
    # Summarize tendencies
    fn_rate = (df_e["tendency"]=="FN-heavy").mean()
    fp_rate = (df_e["tendency"]=="FP-heavy").mean()
    return (
        "<b>Error Analysis.</b> Most performance loss is concentrated in a handful of minority classes; "
        f"{fn_rate:.0%} of low-F1 cases are FN-heavy (low recall), {fp_rate:.0%} FP-heavy (low precision). "
        "Typical FN-heavy classes include nuanced/low-support emotions. Targeted rebalancing and threshold tuning help. "
        "Worst classes per model/scheme (top-3 by lowest F1) were inspected; see raw JSON for details."
    )
...
def make_exec_summary(df):
    ...
    return (
        ...
        "schemes: <b>Raw</b> (multi-label, 28 emotions; sigmoid with a tuned threshold) and <b>Simplified</b> "
        "(single-label, assigning the dominant emotion; softmax). "
        ...
        "Hyperparameter search so far was manual around standard baselines; a systematic grid/random search is planned."
    )

def make_defs_text():
    return (
        "<b>Definitions.</b> Macro-F1 = unweighted mean of per-class F1. Micro-F1 = F1 from global "
        "TP/FP/FN. Weighted-F1 = class-frequency-weighted mean. <i>Raw</i>: sigmoid + threshold (tuned on validation). "
        "<i>Simplified</i>: softmax argmax. Latency: forward-pass time per batch=32 on CPU (tokenization not timed). "
        "<i>Params (M)</i>: trainable parameters in millions. FP32 = standard 32-bit float; INT8 = 8-bit integer (post-training "
        "dynamic quantization) that reduces memory/latency at some accuracy cost."
    )

def make_assessment(df):
    # Identify best models by split/scheme to comment on "best Raw vs best Val" mismatch
    best_text = ""
    try:
        t = df.copy()
        parts = []
        for col, label in [("val_macro","Validation"), ("test_macro","Test")]:
            if col in t and t[col].notna().any():
                row = t.loc[t[col].idxmax()]
                parts.append(f"{label}: <i>{row['model']}</i> ({row.get('scheme','?')})")
        if parts:
            best_text = " Best by split — " + "; ".join(parts) + "."
    except Exception:
        pass

    return (
        "<b>Assessment.</b> Across all four encoders, the Simplified scheme is consistently higher than Raw, "
        "typically +0.16–0.28 Macro-F1 per model. MobileBERT on Simplified delivers the best accuracy (~0.47 Macro-F1) "
        "with moderate latency; MiniLM is fastest but lower accuracy; ALBERT is accurate but slower; DistilBERT is balanced. "
        "Post-training INT8 quantization reduces model size and CPU latency by ~2–3×, with typical Macro-F1 drops of 0.03–0.07 "
        "vs FP32. Use INT8 when latency dominates; prefer FP32 for accuracy-critical use. "
        "Differences in the best model between Validation and Test often come from threshold sensitivity (Raw), "
        "small validation set variance, and mild overfitting; models close in mean F1 can swap rank between splits."
        + best_text
    )
...
def main():
    ...
    if t3 is not None:
        ...
        # New Figure 4 for Table 3
        flow += [Image(fig4, width=480, height=260), Spacer(1,12)]

    # Assessment + Error analysis
    flow += [Paragraph(make_assessment(df), styles["BodyText"]), Spacer(1,8)]
    flow += [Paragraph(make_error_diagnostics(df), styles["BodyText"])]
